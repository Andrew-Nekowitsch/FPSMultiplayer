...
namespace CMF
{
	...
	public class AdvancedWalkerController : Controller
	{
		...
		public float wallWalkCooldown = 1f;
		...
		protected virtual Vector3 CalculateMovementDirection()
		{
			...
			else
			{
				...
				_velocity += Vector3.ProjectOnPlane(cameraTransform.right, tr.up).normalized * input.x;
				_velocity += Vector3.ProjectOnPlane(cameraTransform.forward, tr.up).normalized * input.y;
			}
		...
		private void WallWalkingCooldown()
		{
			if (canChangeWall)
				return;
		...
		public void StartWallWalking()
		{
			canChangeWall = false;
			wallWalkTimer = wallWalkCooldown;
		}
